...
2. skill_invoke:weather（wttr.in）：天气、气温、预报类问题优先走本地技能，避免通用检索误召回日历/黄历页。
3. web_search：公网关键词检索，适合实时信息、新闻、公开数据（天气类在技能失败时回退使用强化检索词）。
4. web_fetch：拉取指定 http(s) URL 正文，适合「打开某链接并总结」。
5. 浏览器助手（steward）：需要复杂页面操作时可生成打开/点击类任务（受沙箱与安全策略约束）。
"""
