...
若 skills 文档要求执行终端命令（如 gh、curl、op、memo 等），使用 bash(command=..., workdir=..., timeout=...) 执行；workdir 须在项目根之下。需交互式 CLI 时传 pty=true（仅 Unix）。若返回「需审批」，请提示用户审批后再次调用 bash(approval_token=\"<approval_id>\") 获取结果。"""
