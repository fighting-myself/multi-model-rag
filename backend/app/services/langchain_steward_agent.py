...
from app.services.steward_tools import get_steward_tools, run_steward_tool, _tool_browser_close, clear_browser_context
...
- page_wait：等待若干秒
- page_list_elements：获取当前页面所有可见可操作元素及推荐 selector（返回「action | label | selector」）。在需要点击或填表前先调用，再根据返回的 selector 调用 page_click/page_fill，可避免盲目写选择器导致点到隐藏元素。
- page_plan_and_act：由 LLM 根据当前页面与子目标决定并执行「一步」动作。传入 goal（如「点击查询按钮」「填写出发地上海」）。适合复杂页面（如 12306、登录页）：page_goto -> page_wait(5~8) -> page_plan_and_act(子目标1) -> page_plan_and_act(子目标2) -> ...
- page_get_text：获取页面可见文本（可选 selector），用于打开文章/新闻页后读取内容并总结
- page_cookies：获取当前页面的 Cookie（JSON），适合在用户登录后执行并返回给用户
- file_write：将文本保存到服务器 data 目录下（path 相对 data，content 为内容）
- skill_list：扫描 skills 目录，返回当前可用技能列表（名称与简介）
- skill_load：按需加载某一技能的完整使用文档（传入 skill_id），使用某技能前应先调用以获取工具名、参数与用法
- browser_close：关闭浏览器

请根据用户指令，按步骤调用上述工具完成任务。

**复杂页面（12306 查票、登录、多链接页）**：优先使用 page_plan_and_act(子目标) 让模型根据真实页面结构选 selector 并执行；或先 page_list_elements 再根据列表中的 selector 调用 page_click/page_fill，不要盲目写 a:has-text('车票') 等易匹配到隐藏项的选择器。若上一步已成功（如「已填入内容」「已点击」），不要重复执行同一子目标，应进行下一步（如填完三项后立即点击查询，再获取结果页文本）。
